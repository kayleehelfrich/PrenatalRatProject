---
title: "Fetal Rat Anthropometry at Collection GD20.5, AIN-76A diet with MD or Alcohol and Iron Supplement or Water- 2 Small Litters Removed- Females Only"
...
# Select out only Female data and equalize the number of rows for each treatment group
```{r}
Females <- data_frame[ which(data_frame$Sex=='F'), ] #Create new dataframe of only Females
```
```{r echo = FALSE}
print(Females)
```
...
Groupcount <- Females %>% count(Treatment) #Calculate how many individual Females there are per treatment group
...
print(Females_new %>% count(Treatment)) #check that the groups are now equal
#rearrange to be in order of Treatment, otherwise the dots get accidentally reassigned to the wrong group
Females_new <- Females_new[order(Females_new$Treatment), ]
```
...
Mean <- tapply(Females_new$Fetal.Wt, as.character(Females_new$Treatment), mean, na.rm=TRUE) #make sure to change selected column to change data for graph
...
Stdev <- tapply(Females_new$Fetal.Wt, as.character(Females_new$Treatment), sd, na.rm=TRUE) #make sure to change selected column
...
dataframe_combine <- cbind(Mean, Stdev, Females_new$Fetal.Wt) #make sure to change selected column
...
#Make Graph#
png("FetalWeight_FemalesOnly_RmData2_PAExFe_Dotplot.png", units="in", width=7, height=7, res=600) 
...
#Mean
Mean <- tapply(Females_new$Placental.Wt, as.character(Females_new$Treatment), mean, na.rm=TRUE) #make sure to change 
Mean <- as.data.frame(rep(Mean, each = NumPoints))
Mean$Treatment <- rep(ActualGroupOrder, each = NumPoints) 
colnames(Mean)[1] <- "Mean"

#Stdev
Stdev <- tapply(Females_new$Placental.Wt, as.character(Females_new$Treatment), sd, na.rm=TRUE) #make sure to change 
Stdev <- as.data.frame(rep(Stdev, each = NumPoints))
Stdev$Treatment <- rep(ActualGroupOrder, each = NumPoints)
colnames(Stdev)[1] <- "Stdev"

#Combine
dataframe_combine <- cbind(Mean, Stdev, Females_new$Placental.Wt) #make sure to change
...
#Make Graph#
png("PlacentalWeight_FemalesOnly_RmData2_PAExFe_Dotplot.png", units="in", width=7, height=7, res=600) 
...
p<- ggplot(dataframe_combine, aes(x=Treatment, y=Mean)) + 
  ...
  ggtitle("Placental Weight- Females") +
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#Mean
Mean <- tapply(Females_new$Placental.Efficiency, as.character(Females_new$Treatment), mean, na.rm=TRUE) #change 
Mean <- as.data.frame(rep(Mean, each = NumPoints))
Mean$Treatment <- rep(ActualGroupOrder, each = NumPoints) 
colnames(Mean)[1] <- "Mean"

#Stdev
Stdev <- tapply(Females_new$Placental.Efficiency, as.character(Females_new$Treatment), sd, na.rm=TRUE) #change 
Stdev <- as.data.frame(rep(Stdev, each = NumPoints))
Stdev$Treatment <- rep(ActualGroupOrder, each = NumPoints)
colnames(Stdev)[1] <- "Stdev"

#Combine
dataframe_combine <- cbind(Mean, Stdev, Females_new$Placental.Efficiency) #make sure to change
colnames(dataframe_combine)[5] <- "Value"
dataframe_combine <- dataframe_combine[-2]
...
#Make Graph#
png("PlacentalEfficiency_FemalesOnly_RmData2_PAExFe_Dotplot.png", units="in", width=7, height=7, res=600) 
...
p<- ggplot(dataframe_combine, aes(x=Treatment, y=Mean)) + 
  ...
  ggtitle("Placental Efficiency- Females") +
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#Mean
Mean <- tapply(Females_new$Brain.Wt, as.character(Females_new$Treatment), mean, na.rm=TRUE) #change 
Mean <- as.data.frame(rep(Mean, each = NumPoints))
Mean$Treatment <- rep(ActualGroupOrder, each = NumPoints) 
colnames(Mean)[1] <- "Mean"

#Stdev
Stdev <- tapply(Females_new$Brain.Wt, as.character(Females_new$Treatment), sd, na.rm=TRUE) #change 
Stdev <- as.data.frame(rep(Stdev, each = NumPoints))
Stdev$Treatment <- rep(ActualGroupOrder, each = NumPoints)
colnames(Stdev)[1] <- "Stdev"

#Combine
dataframe_combine <- cbind(Mean, Stdev, Females_new$Brain.Wt) #make sure to change
colnames(dataframe_combine)[5] <- "Value"
dataframe_combine <- dataframe_combine[-2]
...
#Make Graph#
png("FetalBrainWeight_FemalesOnly_RmData2_PAExFe_Dotplot.png", units="in", width=7, height=7, res=600) 
...
p<- ggplot(dataframe_combine, aes(x=Treatment, y=Mean)) + 
  ...
  ggtitle("Fetal Brain Weight- Females") +
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#Mean
Mean <- tapply(Females_new$Ratio.Brain.Body, as.character(Females_new$Treatment), mean, na.rm=TRUE) #change 
Mean <- as.data.frame(rep(Mean, each = NumPoints))
Mean$Treatment <- rep(ActualGroupOrder, each = NumPoints) 
colnames(Mean)[1] <- "Mean"

#Stdev
Stdev <- tapply(Females_new$Ratio.Brain.Body, as.character(Females_new$Treatment), sd, na.rm=TRUE) #change 
Stdev <- as.data.frame(rep(Stdev, each = NumPoints))
Stdev$Treatment <- rep(ActualGroupOrder, each = NumPoints)
colnames(Stdev)[1] <- "Stdev"

#Combine
dataframe_combine <- cbind(Mean, Stdev, Females_new$Ratio.Brain.Body) #make sure to change
colnames(dataframe_combine)[5] <- "Value"
dataframe_combine <- dataframe_combine[-2]
...
#Make Graph#
png("FetalBraintoBodyWeightRatio_FemalesOnly_RmData2_PAExFe_Dotplot.png", units="in", width=7, height=7, res=600) 
...
p<- ggplot(dataframe_combine, aes(x=Treatment, y=Mean)) + 
  ...
  ggtitle("Fetal Brain to Body Weight Ratio- Females") +
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#Mean
Mean <- tapply(Females_new$Liver.Wt, as.character(Females_new$Treatment), mean, na.rm=TRUE) #change 
Mean <- as.data.frame(rep(Mean, each = NumPoints))
Mean$Treatment <- rep(ActualGroupOrder, each = NumPoints) 
colnames(Mean)[1] <- "Mean"

#Stdev
Stdev <- tapply(Females_new$Liver.Wt, as.character(Females_new$Treatment), sd, na.rm=TRUE) #change 
Stdev <- as.data.frame(rep(Stdev, each = NumPoints))
Stdev$Treatment <- rep(ActualGroupOrder, each = NumPoints)
colnames(Stdev)[1] <- "Stdev"

#Combine
dataframe_combine <- cbind(Mean, Stdev, Females_new$Liver.Wt) #make sure to change
colnames(dataframe_combine)[5] <- "Value"
dataframe_combine <- dataframe_combine[-2]
...
#Make Graph#
png("FetalLiverWeight_FemalesOnly_RmData2_PAExFe_Dotplot.png", units="in", width=7, height=7, res=600) 
...
p<- ggplot(dataframe_combine, aes(x=Treatment, y=Mean)) + 
  ...
  ggtitle("Fetal Liver Weight- Females") +
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#Mean
Mean <- tapply(Females_new$Ratio.Liver.Body, as.character(Females_new$Treatment), mean, na.rm=TRUE) #change 
Mean <- as.data.frame(rep(Mean, each = NumPoints))
Mean$Treatment <- rep(ActualGroupOrder, each = NumPoints) 
colnames(Mean)[1] <- "Mean"

#Stdev
Stdev <- tapply(Females_new$Ratio.Liver.Body, as.character(Females_new$Treatment), sd, na.rm=TRUE) #change 
Stdev <- as.data.frame(rep(Stdev, each = NumPoints))
Stdev$Treatment <- rep(ActualGroupOrder, each = NumPoints)
colnames(Stdev)[1] <- "Stdev"

#Combine
dataframe_combine <- cbind(Mean, Stdev, Females_new$Ratio.Liver.Body) #make sure to change
colnames(dataframe_combine)[5] <- "Value"
dataframe_combine <- dataframe_combine[-2]
...
#Make Graph#
png("FetalLiverBodyWeightRatio_FemalesOnly_RmData2_PAExFe_Dotplot.png", units="in", width=7, height=7, res=600) 
...
p<- ggplot(dataframe_combine, aes(x=Treatment, y=Mean)) + 
  ...
  ggtitle("Fetal Liver to Body Weight Ratio- Females") +
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#Mean
Mean <- tapply(Females_new$Ratio.Brain.Liver, as.character(Females_new$Treatment), mean, na.rm=TRUE) #change 
Mean <- as.data.frame(rep(Mean, each = NumPoints))
Mean$Treatment <- rep(ActualGroupOrder, each = NumPoints) 
colnames(Mean)[1] <- "Mean"

#Stdev
Stdev <- tapply(Females_new$Ratio.Brain.Liver, as.character(Females_new$Treatment), sd, na.rm=TRUE) #change 
Stdev <- as.data.frame(rep(Stdev, each = NumPoints))
Stdev$Treatment <- rep(ActualGroupOrder, each = NumPoints)
colnames(Stdev)[1] <- "Stdev"

#Combine
dataframe_combine <- cbind(Mean, Stdev, Females_new$Ratio.Brain.Liver) #make sure to change
colnames(dataframe_combine)[5] <- "Value"
dataframe_combine <- dataframe_combine[-2]
...
#Make Graph#
png("FetalBraintoLiverWeightRatio_FemalesOnly_RmData2_PAExFe_Dotplot.png", units="in", width=7, height=7, res=600) 
...
p<- ggplot(dataframe_combine, aes(x=Treatment, y=Mean)) + 
  ...
  ggtitle("Fetal Brain to Liver Weight Ratio- Females") +
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#Mean
Mean <- tapply(Females_new$Heart.Wt, as.character(Females_new$Treatment), mean, na.rm=TRUE) #change 
Mean <- as.data.frame(rep(Mean, each = NumPoints))
Mean$Treatment <- rep(ActualGroupOrder, each = NumPoints) 
colnames(Mean)[1] <- "Mean"

#Stdev
Stdev <- tapply(Females_new$Heart.Wt, as.character(Females_new$Treatment), sd, na.rm=TRUE) #change 
Stdev <- as.data.frame(rep(Stdev, each = NumPoints))
Stdev$Treatment <- rep(ActualGroupOrder, each = NumPoints)
colnames(Stdev)[1] <- "Stdev"

#Combine
dataframe_combine <- cbind(Mean, Stdev, Females_new$Heart.Wt) #make sure to change
colnames(dataframe_combine)[5] <- "Value"
dataframe_combine <- dataframe_combine[-2]
...
#Make Graph#
png("FetalHeartWeight_FemalesOnly_RmData2_PAExFe_Dotplot.png", units="in", width=7, height=7, res=600) 
...
p<- ggplot(dataframe_combine, aes(x=Treatment, y=Mean)) + 
  ...
  ggtitle("Fetal Heart Weight- Females") +
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#Mean
Mean <- tapply(Females_new$Ratio.Heart.Body, as.character(Females_new$Treatment), mean, na.rm=TRUE) #change 
Mean <- as.data.frame(rep(Mean, each = NumPoints))
Mean$Treatment <- rep(ActualGroupOrder, each = NumPoints) 
colnames(Mean)[1] <- "Mean"

#Stdev
Stdev <- tapply(Females_new$Ratio.Heart.Body, as.character(Females_new$Treatment), sd, na.rm=TRUE) #change 
Stdev <- as.data.frame(rep(Stdev, each = NumPoints))
Stdev$Treatment <- rep(ActualGroupOrder, each = NumPoints)
colnames(Stdev)[1] <- "Stdev"

#Combine
dataframe_combine <- cbind(Mean, Stdev, Females_new$Ratio.Heart.Body) #make sure to change
colnames(dataframe_combine)[5] <- "Value"
dataframe_combine <- dataframe_combine[-2]
...
#Make Graph#
png("FetalHeartBodyWeightRatio_FemalesOnly_RmData2_PAExFe_Dotplot.png", units="in", width=7, height=7, res=600) 
...
p<- ggplot(dataframe_combine, aes(x=Treatment, y=Mean)) + 
  ...
  ggtitle("Fetal Heart to Body Weight Ratio- Females") +
