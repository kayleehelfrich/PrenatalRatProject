---
title: "Maternal Liver TBARS- Rat Alcohol and Iron Study, GD20.5 Collection, Removing 2 Litters"
...
data_frame <- data_frame[ which(data_frame$Tissue=='Maternal Liver'), ]
...
# Maternal Liver TBARS
```{r, warning = FALSE, message = FALSE}
#Mean- calculate mean for column, repeat it to number of points, and create dataframe
Mean <- tapply(ML$TBARS, as.character(ML$Treatment), mean, na.rm=TRUE) #make sure to change selected column to change data for graph
...
Stdev <- tapply(ML$TBARS, as.character(ML$Treatment), sd, na.rm=TRUE) #make sure to change selected column
...
dataframe_combine <- cbind(Mean, Stdev, ML$TBARS) #make sure to change selected column
...
#Make Graph#
png("MaternalLiver_TBARS_RmData2_PAExFe_Dotplot_B&W.png", units="in", width=7, height=7, res=600) 
...
dev.off() 
```
```{r Maternal Liver TBARS, echo = FALSE}
print(p)
```
