...
setwd("C:/Users/khelfri/Documents/KayleeStuff/Smith_Lab/Data/Rat_Iron/Hepcidin/MaternalLiverHepcidin/Analysis")

#reads a file in a table format and creates a data frame from it
datafile <- "Combined_ML_Hepcidin_Cqs.csv"
...
png("MatLiv_Hepc_ddCtDotPlot_B&W.png", units="in", width=5, height=7, res=600)

sets <- c("CON", "ALC", "CON+Iron", "ALC+Iron")

p<- ggplot(dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean)) + 
  geom_bar(stat="identity", color="black", aes(fill = Biological.Sets), position=position_dodge()) +
  ...
  ggtitle("Maternal Liver Hepcidin") +
...
write.csv(dataframe_AdjCtCalcs,"C:/Users/khelfri/Documents/KayleeStuff/Smith_Lab/Data/Rat_Iron/Hepcidin/MaternalLiverHepcidin/Analysis\\AdjustedCtCalcsForStats.csv", row.names = FALSE)
